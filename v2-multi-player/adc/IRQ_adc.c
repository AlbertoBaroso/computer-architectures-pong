...
void ADC_IRQHandler(void) {
	...
	// current_position GENERATED IS IN RANGE [field_border, field_width - field_border - paddle_width]
	AD_current = ((LPC_ADC->ADGDR >> 4) & 0xFFF); // Read Conversion Result
		...
		if (AD_current != AD_last) {
			...
			unsigned int max_value = field_width - paddle_width - (field_border << 1); // TODO: CHECK IF -1 ALSO NEEDED
			static int current_position = -1;

			current_position = (AD_current * max_value / 0xFFF) + field_border;
			...
			if(measures_filled >= measures_amount - 1) {
				...
				if(time == 0) {
					...
					paddle_bottom_x = avg_similar_measures;
					time++;
					if(paddle_bottom_x <= paddle_width + paddle_width + 1)
						 last_position = field_width - field_border - paddle_width - 2;
					draw_paddle(last_position, paddle_bottom_x, BOTTOM_PLAYER);
					
				} else {
					
					if(positive_value(avg_similar_measures - paddle_bottom_x) >= paddle_threshold) {
						draw_paddle(paddle_bottom_x, avg_similar_measures, BOTTOM_PLAYER);
						paddle_bottom_x = avg_similar_measures;
					}
				}
		...
		AD_last = AD_current;
			...
			if (AD_current != AD_last) {
				...
				int max_value = field_width - paddle_width - (field_border << 1) - 1;
				uint16_t last_position = AD_last * max_value / 0xFFF;
				uint16_t current_position = (AD_current * max_value / 0xFFF) + field_border;
				draw_paddle(last_position, current_position, BOTTOM_PLAYER);
				AD_last = AD_current;
			}
